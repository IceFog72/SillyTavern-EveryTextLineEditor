...
export class HistoryPanel {
    constructor(
        ...
        private delegate: HistoryPanelDelegate
    ) {}
    ...
    render(
        ...
        for (const group of this.getVisibleSourceGroups(activeGroup, allGroups, commits)) {
            ...
            if (group === activeGroup) option.selected = true;
        ...
        const commitBtn = changes.querySelector('.etle--commitButton') as HTMLButtonElement;
    ...
    private groupCommits(commits: HistoryCommit[]): HistoryCommitGroup[] {
        const groups = new Map<string, HistoryCommitGroup>();
    ...
    private getSourceKind(source: TextSource): string {
        if (source.group.includes('World')) return 'WI';
        if (source.group.includes('Persona')) return 'P';
