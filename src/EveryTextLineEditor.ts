...
import { getSpellchecker, MonacoSpellchecker } from './vendor/monaco-spellchecker/spellchecker.es.js';
...
const getStoredSourceLanguages = (): Record<string, string> => {
    try {
...
const getTrackedSourceGroups = (): string[] => {
    try {
...
let typoLoadPromise: Promise<any> | null = null;
let englishDictionaryPromise: Promise<any> | null = null;
...
const loadTypo = async () => {
    if (globalThis.Typo) return globalThis.Typo;
    ...
    typoLoadPromise = new Promise((resolve, reject) => {
        ...
        script.onload = () => {
            if (globalThis.Typo) resolve(globalThis.Typo);
            else reject(new Error('Typo.js loaded, but window.Typo was not found.'));
        };
...
export class EveryTextLineEditor {
    ...
    handleDocumentClick(event) {
        ...
        if (this.dom.root?.contains(target)) return;
    ...
    renderPanel() {
        ...
        const inspectBtn = this.makeTextButton(
            inspectEnabled ? 'Stop Inspecting' : 'Inspect Prompts',
            inspectEnabled ? 'fa-bug-slash' : 'fa-bug',
            () => this.togglePromptInspector(),
        );
    ...
    async clearHistory() {
        const result = await Popup.show.confirm(
            'Clear History',
            'Delete all local ETLE history snapshots from this browser? SillyTavern settings and current prompt text will not be changed.',
            {
    ...
    toggleFullscreen() {
        ...
        if (!isPopup) {
            ...
            this._popupEscHandler = (e: KeyboardEvent) => {
    ...
    togglePromptInspector() {
        ...
        if (btn) {
            ...
            if (label) label.textContent = enabled ? 'Stop Inspecting' : 'Inspect Prompts';
    ...
    addWandToggleButton() {
        ...
        const updateToolbarButton = () => {
            ...
            if (btn) {
                ...
                if (text) text.textContent = isEnabled ? 'Stop Inspecting' : 'Inspect Prompts';
    ...
    async createMonacoEditor(host, value = '', readOnly = false): Promise<PrismEditorLike> {
        ...
        const wrapper: PrismEditorLike = {
            ...
            dispose: () => {
                this.disposeMonacoSpellcheckers();
                if (this.monacoEditor === monacoEditor) this.monacoEditor = null;
    ...
    bindDiffScrollSync() {
        ...
        const handleWheel = (event: WheelEvent) => {
            ...
            if (this.dom.masterScrollbar.scrollTop === before) this.scrollDiffPanesBy(event.deltaY);
        };
    ...
    scrollDiffPanesBy(deltaY: number) {
        if (!this.editor || !this.oldEditor || this.isSyncingScroll) return;
        this.isSyncingScroll = true;
        ...
        if (previousId && visibleSources.some(source => source.id === previousId)) {
            ...
            this.clearSelectedSource(
                this.sources.length
                    ...
                    : 'No editable prompt sources found',
                this.sources.length
    ...
    startSourceEventListeners() {
        if (this.sourceEventHandlers.length) return;
    ...
    async inspectGeneratedPrompt(data: Record<string, any> | undefined, key: 'chat' | 'prompt') {
        if (!data || data.dryRun || localStorage.getItem(STORAGE.promptInspectorEnabled) !== 'true') return;
        ...
        await this.selectSource(sourceId, { force: true });
        if (this.selectedSource?.id !== sourceId) return;
        ...
        if (!wasAlreadyPopup) this.toggleFullscreen();
    ...
    pruneTrackedSources() {
        ...
        if (changed) this.persistTrackedSources();
    }

    async setTrackedSourceGroups(groups: string[]) {
        const availableGroups = new Set(this.sources.map(source => source.group));
        ...
        this.updateStatusBar();
        const visibleSources = this.getTrackedSources();
        if (this.selectedSource && visibleSources.some(source => source.id === this.selectedSource?.id)) return;
    ...
    clearSelectedSource(title: string, detail: string) {
        this.selectedSource = null;
        this.selectedSourceBaseline = '';
        this.selectedSourceChanged = false;
        ...
        this.setEditorValue('');
        this.updateDirty(false);
        if (this.dom.currentTitle) this.dom.currentTitle.textContent = title;
        if (this.dom.currentGroup) this.dom.currentGroup.textContent = detail;
        if (this.dom.actionsLeft) this.dom.actionsLeft.innerHTML = '';
    ...
    getLanguageForSource(source?: TextSource | null): Language {
        if (!source) return this.currentLanguage;
    ...
    setSourceLanguage(source: TextSource, lang: Language) {
    ...
    async confirmPromptSend(isChatCompletion = false) {
        if (!this.dom.actionsLeft) return 'discard';
        ...
        return new Promise<'save' | 'discard' | 'cancel'>((resolve) => {
            const complete = (choice: 'save' | 'discard' | 'cancel') => {
                save.removeEventListener('click', onSave);
                discard.removeEventListener('click', onDiscard);
                cancel.removeEventListener('click', onCancel);
                if (this.dom.actionsLeft?.contains(notice)) this.dom.actionsLeft.innerHTML = '';
                resolve(choice);
            };
    ...
    withSuppressedEditorChange(callback: () => void) {
    ...
    stopSourceWatcher() {
        if (this.sourceWatchTimer === null) return;
        window.clearInterval(this.sourceWatchTimer);
        this.sourceWatchTimer = null;
    }

    async refreshSelectedSourceBaseline() {
        if (!this.selectedSource || this.historyCommit?.sourceId === this.selectedSource.id) return;
        if (this.sourceWatchInFlight) return;
        ...
        if (this.selectedSource?.id !== watchedSource.id) return;
        if (liveValue === this.selectedSourceBaseline) return;
        if (watchedSource.id.startsWith('current-character-card:')) {
            await this.refreshSources(true);
            return;
        }
    ...
    setIgnoreFullJsonHistory(enabled: boolean) {
    ...
    applySpellCheckToMonaco(root?: HTMLElement | null) {
        if (!root) return;
        root.querySelectorAll<HTMLTextAreaElement>('textarea, .inputarea').forEach(textarea => this.applySpellCheckToTextArea(textarea));
    }

    observeMonacoSpellCheck(root?: HTMLElement | null) {
        if (!root || root.dataset.etleSpellObserver === '1') return;
        root.dataset.etleSpellObserver = '1';
        const observer = new MutationObserver(() => this.applySpellCheckToMonaco(root));
        observer.observe(root, { childList: true, subtree: true });
    }

    async setupMonacoSpellchecker(monacoEditor?: any) {
        if (!isSpellCheckEnabled() || !monacoEditor?.getModel) return;
        ...
        const spellchecker = getSpellchecker(monaco, monacoEditor, {
            ...
            check: (word: string) => {
                ...
                if (this.shouldIgnoreSpellWord(normalized)) return true;
                return this.monacoUserDictionary.has(normalized) || dictionary.check(word);
            },
            suggest: (word: string) => dictionary.suggest(word).slice(0, 8),
            ignore: (word: string) => {
            ...
            tokenize: (line: string) => this.tokenizeSpellcheckLine(line),
            messageBuilder: (type: string, word: string) => {
                if (type === 'hover-message') return `"${word}" may be misspelled.`;
                if (type === 'ignore') return `Ignore "${word}"`;
                if (type === 'add-word') return `Add "${word}" to this session`;
    ...
    scheduleMonacoSpellcheck() {
        if (!isSpellCheckEnabled() || !this.monacoSpellcheckers.length || this.monacoSpellcheckFrame !== null) return;
    ...
    *tokenizeSpellcheckLine(line: string): Iterable<{ word: string; pos: number }> {
    ...
    shouldIgnoreSpellWord(word: string) {
    ...
    setLanguage(lang: Language) {
        this.currentLanguage = lang;
        if (this.selectedSource) this.setSourceLanguage(this.selectedSource, lang);
        this.editor?.setOptions({ language: lang.id });
        this.oldEditor?.setOptions({ language: lang.id });
        this.setMonacoDiffLanguage();
        this.updateStatusBar();
    ...
    updateHeader() {
        ...
        if (this.dom.actionsLeft) {
            ...
            if (this.dom.actionsLeft.hidden) return;
    ...
    updateStatusBar() {
        ...
        this.dom.settingsPanel?.querySelector<HTMLButtonElement>('[data-setting="ignoreFullJsonHistory"]')?.classList.toggle('etle--activeButton', isFullJsonHistoryIgnored());
        const minimapSetting = this.dom.settingsPanel?.querySelector<HTMLButtonElement>('[data-setting="minimap"]');
    ...
    getDiffOriginalValue() {
        ...
        if (commit && commit.sourceId === this.selectedSource?.id) return commit.content;
    ...
    updateDiffSideLabels() {
        ...
        if (this.dom.oldDiffLabel) this.dom.oldDiffLabel.textContent = labels.left;
        if (this.dom.editorDiffLabel) this.dom.editorDiffLabel.textContent = labels.right;
        if (this.dom.monacoDiffOriginalLabel) this.dom.monacoDiffOriginalLabel.textContent = labels.left;
        if (this.dom.monacoDiffModifiedLabel) this.dom.monacoDiffModifiedLabel.textContent = labels.right;
    }
    ...
    async openMonacoDiff() {
        if (!this.dom.monacoDiffEditorHost || !this.selectedSource) return;
        this.dom.root?.classList.add('etle--monacoDiffMode');
    ...
    disposeMonacoDiff({ syncValue = true, clearHost = false } = {}) {
        ...
        this.monacoDiffOriginalModel = null;
        this.monacoDiffModifiedModel = null;
        if (clearHost && this.dom.monacoDiffEditorHost) this.dom.monacoDiffEditorHost.innerHTML = '';
    }

    layoutMonacoDiff(focus = false) {
        if (!this.monacoDiffEditor || !this.dom.monacoDiffEditorHost) return;
    ...
    async refreshHistory() {
        ...
        await Promise.all(groupSources.map(async (source) => {
            if (source.readonly || source.placeholder || this.isSourceIgnoredForHistory(source)) return;
